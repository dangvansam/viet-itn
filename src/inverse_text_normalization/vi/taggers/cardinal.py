...
class CardinalFst(GraphFst):
    ...
    def __init__(self):
        ...
        # Luôn ánh xạ 'năm' thành '5' ở vị trí hàng đơn vị
        last_digit = pynini.union(
            graph_digit,    # bao gồm 'năm' -> '5'
            graph_one,
            graph_four,
            graph_five,
        )
        ...
        graph = pynini.union(
            graph_billion + delete_space + graph_million + delete_space + graph_thousands + delete_space + graph_hundred_ties_zero,
            graph_ten_thousand + delete_space + graph_ten_thousand_suffix + delete_space + graph_hundred_ties_zero,
            graph_hundred_component_at_least_one_none_zero_digit + delete_space + pynutil.delete(pynini.union("nghìn", "ngàn")) + delete_space + (((last_digit | graph_half) + pynutil.insert("00")) | graph_hundred_ties_zero),
            graph_digit,
            graph_zero,
        )
